...
import { Users, Plus, ArrowUpRight, Search, X } from 'lucide-react'
...
export default function ClientsClient({ clients, de }: Props) {
    ...
    <div className="space-y-8 animate-in">
      ...
      {clients.length > 0 && (
        <div className="relative max-w-sm">
          ...
          <input
            ...
            placeholder={de ? 'Kunde suchen…' : 'Search clients…'}
            className="w-full pl-10 pr-9 py-2.5 rounded-xl text-[13.5px] transition-all focus:outline-none"
            style={{
      ...
      {clients.length > 0 ? (
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <p className="text-[13.5px] font-medium" style={{ color: 'var(--text-muted)' }}>
                        {de ? `Kein Kunde gefunden für „${query}"` : `No client found for "${query}"`}
                      </p>
                    </td>
                  </tr>
                ) : (
                  filtered.map((client, idx) => {
                    ...
                    const av = avatarColors[clients.indexOf(client) % avatarColors.length]
        ...
        <div
          ...
          style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', boxShadow: 'var(--card-shadow)' }}
        >
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-5" style={{ background: 'rgba(59,130,246,0.08)' }}>
